...
PCA is a tool used to describe a data set in terms of its intrinsic directions of highest covariance. Consider a set $S$ of $m$ points in $\R^n$. Then $S$ can be represented as an $m \times n$ matrix $X$. Call $\mu_X = (\mu_1, ..., \mu_n)$ the center of the data set, or the average of each of the columns of $X$. The general sequence of steps to perform a PCA on $X$ is as follows:
\begin{enumerate}[(1)]
\item Compute the covariance matrix $\Sigma$ associated to the data set $X$, where $\Sigma_{ij} = \text{cov}(X_{*i}, X_{*j}) =(1/n) \sum_{k=1}^n (X_{ki} - \mu_i)(X_{kj} - \mu_j).$ Notice that this is just the dot product of the mean-centered column vectors of the data matrix divided by the dimension of the space.
